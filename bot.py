...
def Advanced_metadata(URL):
    ...
    info["disableMK"] = (
        not data.get("disableMK", False)
        if isinstance(data.get("disableMK"), bool)
        else data.get("disableMK", "")
    )
    info["disablePowers"] = (
        not data.get("disablePowers", False)
        if isinstance(data.get("disablePowers"), bool)
        else data.get("disablePowers", "")
    )
    info["disableSelling"] = (
        not data.get("disableSelling", False)
        if isinstance(data.get("disableSelling"), bool)
        else data.get("disableSelling", "")
    )
    info["startingCash"] = data.get("startingCash", "")
    info["abilityCooldownReductionMultiplier"] = data.get(
    ...
    info["regrow_rate"] = data.get("_bloonModifiers", {}).get(
    ...
    info["ceramic_health"] = (
        data.get("_bloonModifiers", {}).get("healthMultipliers", "").get("bloons", "")
    )
    info["moab_health"] = (
        data.get("_bloonModifiers", {}).get("healthMultipliers", "").get("moabs", "")
    )
    ...
    for tower in tmp:
        ...
        if tower_name in towers:
            ...
            total_tiers = 5
            towers[tower_name]["path1NumBlockedTiers"] = total_tiers - (
                towers[tower_name]["path1NumBlockedTiers"]
                if towers[tower_name]["path1NumBlockedTiers"] != -1
                else 5
            )
            towers[tower_name]["path2NumBlockedTiers"] = total_tiers - (
                towers[tower_name]["path2NumBlockedTiers"]
                if towers[tower_name]["path2NumBlockedTiers"] != -1
                else 5
            )
            towers[tower_name]["path3NumBlockedTiers"] = total_tiers - (
                towers[tower_name]["path3NumBlockedTiers"]
                if towers[tower_name]["path3NumBlockedTiers"] != -1
                else 5
            )
    ...
    try:
        ...
        if result:
            ...
            if last_advanced_id != id_:
                ...
                embed.add_field(
                    name="난이도",
                    value=f"{info['diff']} {info['diff_mode']}",
                    inline=True,
                )
                embed.add_field(name="활성화", value="", inline=False)
                embed.add_field(
                    name="더블 캐시", value=info["disableDoubleCash"], inline=True
                )
                embed.add_field(
                    name="인스타 몽키", value=info["disableInstas"], inline=True
                )
                ...
                embed.add_field(
                    name="시작 캐시", value=info["startingCash"], inline=False
                )
                embed.add_field(
                    name="시작 라운드", value=info["startRound"], inline=True
                )
                ...
                embed.add_field(
                    name="최대 캐시", value=info["leastCashUsed"], inline=True
                )
                embed.add_field(
                    name="최대 티어", value=info["leastTiersUsed"], inline=True
                )
                embed.add_field(name="최대 타워", value=info["maxTowers"], inline=True)
                embed.set_footer(
                    text=f"알림 시간 : {datetime.today().strftime('%Y/%m/%d %H:%M:%S')}"
                )
                ...
                embed.add_field(
                    name="풍선 속도", value=info["bloon_speed"], inline=True
                )
                embed.add_field(name="MOAB 속도", value=info["moab_speed"], inline=True)
                embed.add_field(
                    name="세라믹 체력", value=info["ceramic_health"], inline=True
                )
                embed.add_field(
                    name="MOAB 체력", value=info["moab_health"], inline=True
                )
                embed.add_field(
                    name="재생 속도", value=info["regrow_rate"], inline=True
                )
                embed.add_field(
                    name="쿨타임",
                    value=info["abilityCooldownReductionMultiplier"],
                    inline=True,
                )
                embed.add_field(
                    name="장애물 비용",
                    value=info["removeableCostMultiplier"],
                    inline=True,
                )
                if avaliable_heros:
                    embed.add_field(
                        name="사용 영웅", value=avaliable_heros, inline=False
                    )
                embed.set_footer(
                    text=f"알림 시간 : {datetime.today().strftime('%Y/%m/%d %H:%M:%S')}"
                )
                ...
                embed = discord.Embed(
                    title="사용 타워", description="오늘의 고급도전은?", color=0x00D4FF
                )
