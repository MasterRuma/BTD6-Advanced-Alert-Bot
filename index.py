...
def Advanced_metadata(URL):
    ...
    for hero in tmp:
        hero_name = hero['tower']  # 현재 영웅의 이름을 변수에 저장
        if hero_name in heros:     # heros에 해당 영웅이 있는지 확인
            heros[hero_name] = hero['max']  # 점수 업데이트
    available_hero()
    ...
    for tower in tmp:
        ...
        if tower_name in towers:
            towers[tower_name]['max'] = tower['max']  # tmp가 아닌 현재 tower 사용
        ...
        if result:
            ...
            if last_advanced_id != id_:
                ...
                for tower_key, tower_name in tower_names.items():
                    ...
                    # 'max' 값이 0이 아닐 때는 이름과 max 값도 함께 출력
